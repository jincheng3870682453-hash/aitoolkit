...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
修复记录：
1. 删除 mojibake_patterns 默认值中的空格 ' '，避免正常输入被误杀
2. 新增 sanitize_ai_code() 清洗 AI 返回的 Markdown 代码块，解决"TXT和代码分不清"
3. ai_planner JSON 提取改用非贪婪匹配 + 代码块优先提取，避免解析失败
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
class Context:
    ...
    # 修复：删除默认值中的空格 ' '，避免正常输入被误杀
    mojibake_patterns: List[str] = field(default_factory=lambda: ['\ufffd', '锟斤拷', '\x80', '\x81', '\x82', '\x83'])
    debug: bool = False
...
def load_config(config_path: str) -> dict:
    default = {
        ...
        # 修复：删除默认值中的空格 ' '
        "mojibake_patterns": ['\ufffd', '锟斤拷', '\x80', '\x81', '\x82', '\x83'],
        "allow_symlink": False,
        "smoke_timeout": 30,
...
# 新增：AI 返回内容清洗函数 —— 解决 "TXT 和代码分不清"
def sanitize_ai_code(text: str) -> str:
    """从 AI 返回的 Markdown 代码块中提取纯代码，过滤解释性文本"""
    if not text:
        return ""
    ...
    code_lines = []
    skip_prefixes = ('以下是', '这是', '修复后的', '注意：', '以下是修复后的代码', '以下是代码')
    for l in lines:
        stripped = l.strip()
...
def ai_planner(user_input: str, error_log: str, ctx: Context) -> Tuple[List[str], Dict[str, str]]:
    ...
    try:
        ...
        # 修复：先清洗可能包裹在 markdown 里的 JSON
        raw = sanitize_ai_code(raw)
...
请修复这段代码中的错误，直接输出修复后的完整代码，不要添加任何解释。"""
                ...
                try:
                    raw_response = call_ai_api(fix_prompt, ctx)
                    # 修复：清洗 AI 返回的 Markdown 代码块，提取纯代码
                    final_code = sanitize_ai_code(raw_response)

                    # 空代码保护
                    if not final_code or len(final_code.strip()) < 10:
                        raise RuntimeError("AI返回内容过短，无法解析为有效代码")
                except Exception as e:
...
def main():
    ...
    # 修复：default_mojibake 中也不包含空格
    default_mojibake = ['\ufffd', '锟斤拷', '\x80', '\x81', '\x82', '\x83']
